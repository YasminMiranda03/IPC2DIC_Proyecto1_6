...
class GenerarXML:
    ...
    def generar_salida(self, nombre_archivo="salida.xml"):
        try:
            carpeta = "reportes"
    ...
    def _agregar_estado_centros(self, root):
        ...
        while actual is not None:
            ...
            usado_cpu = centro.cpu_total - centro.cpu_disponible
            cpu_util_pct = (usado_cpu / centro.cpu_total * 100) if centro.cpu_total > 0 else 0
            cpu_util.text = f"{cpu_util_pct:.2f}%"
            ...
            usado_ram = centro.ram_total - centro.ram_disponible
            ram_util_pct = (usado_ram / centro.ram_total * 100) if centro.ram_total > 0 else 0
            ram_util.text = f"{ram_util_pct:.2f}%"
            ...
            while vm_actual is not None:
                contador_vms += 1
                total_contenedores += vm_actual.dato.contenedores.size
                vm_actual = vm_actual.siguiente

            cant_vms = ET.SubElement(centro_elem, 'cantidadVMs')
            cant_vms.text = str(contador_vms)
